@extends('layouts.dashboard')

@section('title', 'Yangi mijoz')
@section('breadcrumb', 'Marketing')
@section('header_title', 'Yangi mijoz')

@section('content')
<div class="max-w-3xl mx-auto">

    <a href="{{ route('leads.index') }}"
       class="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--text-muted)] hover:text-white transition-colors mb-6">
        <x-lucide-arrow-left class="w-4 h-4" />
        Mijozlarga qaytish
    </a>

    <div class="card p-6 sm:p-8">
        <div class="mb-8 pb-6 border-b border-[var(--border-subtle)]">
            <h2 class="text-xl font-bold text-white tracking-tight">Yangi mijoz qo'shish</h2>
            <p class="text-sm text-[var(--text-muted)] mt-1">Mijoz ma'lumotlarini kiriting</p>
        </div>

        <form action="{{ route('leads.store') }}" method="POST" novalidate>
            @csrf
            @include('leads._form', ['lead' => null])

            <div class="flex items-center justify-end gap-3 pt-6 border-t border-[var(--border-subtle)]">
                <a href="{{ route('leads.index') }}" class="btn-secondary">Bekor qilish</a>
                <button type="submit" class="btn-primary">
                    <x-lucide-user-plus class="w-4 h-4" />
                    Mijoz qo'shish
                </button>
            </div>
        </form>
    </div>
</div>
@endsection
